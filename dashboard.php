...
<?php
   ...
   foreach ($result as $row){
      ...
      echo '<h5 class="form-controld dform-control-user" id="pin'.$row['id_account'].'">'.printValue($row['Pin']).'</h5>';
      echo '</div>';
...
<?php
   ...
   if(isset($_POST['add'])){
      ...
      // mysql select query
      $str="INSERT INTO accounts(KeyC, Title, Username, Passwd, Pin, Url, Tag, Icon, Color, Notes) VALUES('$encryptionKey', '$title', '$usernameEncrypted', '$passwordEncrypted', '$pin', '$url' ,'$tag' ,'$icon', '$color', '$notes');";
   ...
   if(isset($_POST['update'])){
      ...
      // mysql select query
      $str="UPDATE accounts SET Title = '$title', Username = '$usernameEncrypted', Passwd = '$passwordEncrypted', Pin = '$pin', Url = '$url', Tag = '$tag', Icon = '$icon', Color = '$color', Notes = '$notes' WHERE id_account = '$id_account';";
